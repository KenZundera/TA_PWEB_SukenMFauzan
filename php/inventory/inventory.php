...
$inventory = $conn->query(
    'SELECT player.id_player, player.username, inventory.* FROM inventory, player WHERE inventory.id_inventory = player.id_inventory ORDER BY inventory.id_inventory ASC LIMIT ' .
        $halamanawal .
        ',' .
...
<body class="p-2 hidden">
    ...
    <nav class="navbar">
      ...
      <ul>
        ...
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle active" href="" role="button" data-bs-toggle="dropdown" aria-expanded="false"> Inventory </a>
    ...
    <section class="content" id="content">
        <marquee behavior="alternate" direction=""><h1>Inventory User Epic RPG</h1></marquee>
...
?>
        <div id="container1">
            <table class="" cellpadding="15" cellspacing="0" id="data-table" class="data-table">
                ...
                <?php while ($show = mysqli_fetch_array($inventory)) { ?>
                    <!-- <tbody> -->
    ...
    <!-- Live Search JS -->
    <script src="searchInventory.js"></script>
    ...
    <script>
        $(document).on('click', '#tbl-edit', function(e) {
            ...
            // Inventory
            let log = $(this).closest('tr').find('#log').text();
            let fish = $(this).closest('tr').find('#fish').text();
            let fruit = $(this).closest('tr').find('#fruit').text();
            let drop_item = $(this).closest('tr').find('#drop_item').text();
            let potion = $(this).closest('tr').find('#potion').text();
            let cookie = $(this).closest('tr').find('#cookie').text();
            let lootbox = $(this).closest('tr').find('#lootbox').text();
            let seed = $(this).closest('tr').find('#seed').text();
            let epic_item = $(this).closest('tr').find('#epic_item').text();
