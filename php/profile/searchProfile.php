...
$sql = "SELECT player.*, profile.* 

    FROM profile, player

    WHERE profile.id_profile = player.id_profile AND
       (
        ...
        profile.armor LIKE '%$keyword%'
        )
    ORDER BY profile.id_profile ASC
    ";
...
<?php else: ?>
    <table class="" cellpadding="15" cellspacing="0" id="data-table" class="data-table">
                ...
                while ($show = mysqli_fetch_array($profile)) { ?>
                    <!-- <tbody> -->
